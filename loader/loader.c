...
/*
unsigned char *system_variables;
unsigned char *system_variables_info;

unsigned char *system_variables_enum;
unsigned char *system_variables_enum_info;

unsigned char *system_info;
*/
...
/*
void cprintk(unsigned char c, const char *msg, ...)
{
	int slen = strlen(msg)*4+1;
	char msgBuf[1024];// = (char*)malloc(slen);
	va_list va_alist;
...
unsigned long storage_read(int id, void *buffer, int sector, int count)
{
	...
	switch (storage_drive_controller)
	{
		...
		case STORAGE_CONTROLLER_EHCI:
		{
			...
			//r = ehci_msc_read_sector(id, sector, count, SECTORSIZE, buffer);			
		}
		break;
		case 0:
		{
...
#ifdef reload
#define KHEAP_START 0x00100000 // 1 MB mark, or safe location past your kernel binary
#define KHEAP_SIZE  0x00400000 // 4 MB of heap space

static unsigned long kheap_begin = 0;
static unsigned long kheap_end = 0;
static unsigned long kheap_current = 0;

void kmalloc_init(unsigned long kheap_address, size_t kheap_size) 
{
    kheap_begin = kheap_address;
    kheap_end = kheap_address + kheap_size;
    kheap_current = kheap_begin;
}
...
void jump_to(unsigned long address)
{
    void (*entry)(void) = (void (*)(void))(unsigned long)address;
    entry();
...
{
	system_variables = (unsigned char *)malloc(8000);
	system_variables_info = (unsigned char *)malloc(8000);
	system_variables_enum = (unsigned char *)malloc(8000);
	system_variables_enum_info = (unsigned char *)malloc(8000);	
	system_info = (unsigned char *)malloc(8000);	
}
*/
...
void loadloader(void)
{
	unsigned long p_lba = 0;
	unsigned char has_fat16=0;
	unsigned char has_fat32=0;
	unsigned char *part_ptr = (unsigned char*)0x81BE;
	partition_entry_t *p = (partition_entry_t*)part_ptr;
	partition_entry_t *pa = NULL;
	unsigned char *fat_ptr = (unsigned char*)0x7C00;
	fat_t *p_fat = (fat_t*)fat_ptr;
	storage_drive_controller = STORAGE_CONTROLLER_IDE;
    kmalloc_init(KHEAP_START, KHEAP_SIZE);
	//char t1[12];
	//memset(t1, 0, 12);
	//memcpy(t1, p_fat->oem_name, 8);
	//printk("OEM NAME: %s\n", t1);
	for(int i=0;i<4;i++)
	{
		if (p[i].flag & PARTITION_ACTIVE)
		{
			switch(p[i].type)
			{
				case PARTITION_FAT16:
	...
	if (pa != NULL)
	{
		if (has_fat16 || has_fat32) p_lba = pa->lba_start;
	}
	unsigned long loader_size=0;
	unsigned long need_sectors=0;
	unsigned long kernel_sector=0;
	char loader_fn11l[16];
	memset(loader_fn11l, 0, 16);
	memset(loader_fn11l, ' ', 11);
	memcpy(loader_fn11l, loaderfilename, strlen(loaderfilename));
	loader_fn11l[strlen(loaderfilename)] = ' ';
	loader_fn11l[strlen(loaderfilename)+1] = ' ';
	char loader_fn11[12];
	memset(loader_fn11, 0, 12);
	memcpy(loader_fn11, strupr(loader_fn11l), 11);
	loader_fn11[12] = '\0';
	unsigned char *root_dir = (unsigned char*)&root_sector[0];
	unsigned long root_offs = 0;
	char d_fn[12];
	memset(d_fn, 0, 12);	
	for(int i=0;i<16;i++)
	{
		unsigned char *d_root_ptr = (unsigned char*)root_dir+root_offs;
		file_entry_t *e = (file_entry_t*)d_root_ptr;
		loader_size = e->size;
		need_sectors = (loader_size / 512) + 1;
		memset(d_fn, 0, 12);		
		strncpy(d_fn, d_root_ptr, 11);
		d_fn[12] = '\0';
		if (strncmp(d_fn, loader_fn11, 11) == 0) 
		{
			unsigned short cluster_hi = e->first_cluster_hi;
			unsigned short cluster_lo = e->first_cluster_lo;
			unsigned long cluster = USHORT16(cluster_hi, cluster_lo);
			unsigned long first_sector;
			unsigned long data_sector;
			unsigned long sector_per_cluster;
			unsigned long start;
			...
			unsigned long entries;
			unsigned long bytes;
			sector_per_cluster = p_fat->bpb.bpb1.sector_per_cluster;	
			start = p_lba + p_fat->bpb.bpb1.reserved_sectors_count;
			if (has_fat16)
			{
				fat_size = p_fat->bpb.bpb1.fat_size_16;
			}
			else if (has_fat32)
			{
				fat_size = p_fat->bpb.bpb2.fat32.fat_size_32;
			}
			sectors = fat_size * p_fat->bpb.bpb1.number_fats;
			root_start = start + sectors;
			bytes = p_fat->bpb.bpb1.bytes_per_sector;
			entries = p_fat->bpb.bpb1.root_entries_count;
			root_count = (FAT_ENTRY_SIZE * entries + bytes - 1) / bytes;
			data_sector = (root_start + root_count);
			kernel_sector = (data_sector + (cluster-2) * sector_per_cluster);
			break;
		}
		root_offs += 32;
	}
	//while(1);
	//readsector(unsigned long sector, unsigned char *buffer);
	//void dump_hex(const void *data, size_t size)
	//0xC0000000
	//printk("Loader size: %d bytes.\n", loader_size);
	//printk("Need for read size: %d sectors.\n", need_sectors);
	unsigned char *lsector = (unsigned char*)kmalloc(loader_size+512);
	//while(1);
	unsigned long kernel_start_addr = 0xa000;
	unsigned long kernel_sector_remain = 127;
	unsigned long kernel_remain_addr = (kernel_sector_remain * 512);
	unsigned long n_sec = 0;
	for(int i=0;i<need_sectors;i++)
	{
		n_sec = kernel_sector+i; //kernel_sector+kernel_sector_remain+i;
		unsigned long pos = (i*512);
		//printk("Position: 0x%08X\n", pos);
		//printk("Position: 0x%08X\n", (n_sec*512));
		unsigned char *sector = (unsigned char*)(lsector + pos);
		readsector(n_sec, sector);
		//dump_hex(sector, 32);
		//printk("\n");
		//printk("LBA Position: 0x%08X, ", (n_sec*512));
		//printk("Kernel Position: 0x%08X\n", (unsigned long)sector);
	}
	last_eip_1=get_eip();
	void *ptr = (void*)lsector+last_eip_1;//(last_eip_1 - (unsigned long)lsector);
	if (jmp_1 == 1)
	{
		jmp_1 = 0;
		//printk("Last: 0x%08X\n", lsector);
		//printk("Last EIP: 0x%08X\n", last_eip_1);
		//dump_hex(ptr, 32);
		//printk("Next Address: 0x%08X\n", (unsigned long)ptr);
		//printk("Next Address: 0x%X\n", (unsigned long)ptr);
		jump_to(last_eip_1);
	}
	//printk("Next...\n");
}
...
int main(void)
{
	...
	if (physical_memory < 256)
	{
		printk("Error: FastSystem requires minimum >= %d MB of memory.\n", 256);
		panic((unsigned long)info);
	}
