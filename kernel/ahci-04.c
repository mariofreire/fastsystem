...
bool ahci_identify(ahci_hba_port_t *port, uint16_t *buf)
{
	int i=0,sp=0;
	port->is = (uint32_t)-1;
...
unsigned char get_ahci_name(ahci_hba_port_t *port, char *s)
{
	unsigned long i;
	unsigned char st = 0;
	...
	if ((dt == AHCI_DEV_SATA) || (dt == AHCI_DEV_SATAPI) || 
		(dt == AHCI_DEV_SEMB) || (dt == AHCI_DEV_PM))
	{
		if (ahci_identify(port, (uint16_t*)&buf[0]))
		{
			for(i=0;i<512;i+=2) 
			{
				st = b[i];
