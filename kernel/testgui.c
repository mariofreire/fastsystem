...
uint32_t getpixel(int x, int y)
{
	...
	uint32_t p = (unsigned long)(*(unsigned long *)(video_memory + xyoffset24(x, y, v_mode->scan_line_size)) & 0xFFFFFFFF);
	uint32_t c = UINT32(UCHAR8C(p), UCHAR8B(p), UCHAR8D(p), 0);
	return c;
}
...
void drawpixel(int x, int y, uint32_t c)
{
	...
	if (c & 1) color = UINT32(GetRValue(c),GetGValue(c),GetBValue(c), 0xFF);
	else 
	{
		if (c == 0) color = UINT32(GetRValue(c),GetGValue(c),GetBValue(c), 0x00);
		else color = UINT32(GetRValue(c),GetGValue(c),GetBValue(c), 0xFF);
	}
	(*(unsigned long *)(video_memory + xyoffset24(x, y, v_mode->scan_line_size))) = color & 0xFFFFFFFF;
	//(*(unsigned long *)(video_memory + xyoffset24(x, y, v_mode->scan_line_size))) = ((0xFF<<24 | RGB(GetRValue(c),GetGValue(c),GetBValue(c))) & 0xFFFFFFFF);
}
...
void drawcursor(int x, int y)
{
	for(int yy=0;yy<ARROW_HEIGHT;yy++)
	{
		for(int xx=0;xx<ARROW_WIDTH;xx++)
		{
			...
			if (t == 0) 
			{
				setpixel(x+xx+1, y+yy+1, clGray, 96);
...
void resetkeys()
{
	__asm__ volatile ( "int $0x80" : : "a" (517), "b" (0), "c" (0), "d" (0) );
}
...
int main() 
{
	...
	fillrect(100, 360, 100+64, 360+64, clBlack, 255);
