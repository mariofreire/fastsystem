...
static unsigned char frame_bitmap[TOTAL_FRAMES / 8];
...
{	
	...
	/*
    for (i = 0; i < 16; i++) {
        page_directory[i] = (i * 0x400000) | PAGE_PRESENT | PAGE_READWRITE | PAGE_USER | PAGE_PAGESIZE;
        page_directory[768 + i] = (i * 0x400000) | PAGE_PRESENT | PAGE_READWRITE | PAGE_USER | PAGE_PAGESIZE;
    }
    */
}
...
{
    clear_frame((unsigned long)physaddr);
}
...
void map_page_1(void *physaddr, void *virtualaddr, unsigned int flags) 
{
    unsigned long pdindex = (unsigned long)virtualaddr >> 22;
    unsigned long ptindex = (unsigned long)virtualaddr >> 12 & 0x03FF;
    ...
    flushpage((unsigned long)virtualaddr);
}
...
void map_page(void *physaddr, void *virtualaddr, unsigned int flags) 
{
    ...
    if (!(pd[pdindex] & PAGE_PRESENT)) 
    {
        ...
        unsigned long *pt_virt = (unsigned long *)new_pt; 
        for (int i = 0; i < 1024; i++) 
        {
            pt_virt[i] = 0;
        }

        pd[pdindex] = ((unsigned long)new_pt) | PAGE_PRESENT | PAGE_READWRITE | PAGE_USER;
    }

    unsigned long *pt = (unsigned long *)(pd[pdindex] & ~0xFFF);
...
void unmap_page(void *virtualaddr) 
{
    ...
    unsigned long pd_entry = page_directory[pdindex];
    if (!(pd_entry & PAGE_PRESENT)) return;
    unsigned long *pt = (unsigned long *)(pd_entry & 0xFFFFF000);
    pt[ptindex] = 0;

    flushpage((unsigned long)virtualaddr);
}

unsigned long placement_address = (unsigned long)&kernel_end; // ((unsigned long)&kernel_end) + 0x10000;
...
void page_fault(registers_t *registers)
{
	...
	if (enter_kernelmode == 1) 
	{
		...
		{
			if (current_thread != 0) sys_exit(0);
		}
		return;
	}
