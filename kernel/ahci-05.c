...
int check_ports(uint8_t *ports, uint8_t *portmax);
int init_ports();
				...
				if (pci_device[i].pci.vendor != 0xFFFF)
				{
					if ((pci_device[i].pci.class == 0x01) && (pci_device[i].pci.subclass == 0x06))
					{
						if (pci_device[i].pci.bar[5] != 0)
						{
							...
							ahci->version = 1;
							portcnt = check_ports(&ahci->list[0], &ahci->count);
...
int check_ports(uint8_t *ports, uint8_t *portmax)
{
	...
	while (i < 32)
	{
		if (pi & 1)
		{
			int dt = check_type(&ahci_hba->ports[i]);
			if (dt == AHCI_DEV_SATA)
			{
				ports[portcount] = i;
				portcount++;
...
int init_ports()
{
	...
	while (i < 32)
	{
		if (pi & 1)
		{
			...
			{
				ahci->port[i].type = dt;
				port_rebase(&ahci_hba->ports[i], i);
				if (f == 0)
				{
...
bool get_sata_ident(ahci_hba_port_t *port, void *buffer)
{
	int i=0,sp=0;
	port->is = (uint32_t)-1;
	...
	cmdfis->c = 1;
	if (dt == AHCI_DEV_SATAPI) cmdfis->command = ATA_CMD_IDENTIFY_PACKET;
	else cmdfis->command = ATA_CMD_IDENTIFY;
	cmdfis->device = 0;
	cmdfis->countl = 1;
	cmdfis->counth = 0;
	while ((port->tfd & (ATA_DEV_BUSY|ATA_DEV_DRQ)) && (sp<1000000)) sp++;
	if (sp==1000000) return false;
	port->ci = 1<<slot;
	while (1)
	{
