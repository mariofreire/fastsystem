...
bool loadfat16(void)
{
	if (!imagedisk_file) return false;
	if (!hasactive()) return false;
	if (!isfat16type()) return false;	
	readsector(main_partition->lba_start, BOOT);
	fat16 = (fat16_t*)BOOT;
	if (fat16->signature != BOOT_SIGNATURE) return false;
	memcpy(bootstrap, fat16->bootstrap, FAT16_BOOTSTRAP_SIZE);	
	return true;
}
...
uint32_t getrootdirsector(void)
{
	...
	if (!isfat16type()) return 0;
	start = main_partition->lba_start + fat16->reserved_sectors_count;
	sectors = fat16->fat_size_16 * fat16->number_fats;
...
uint32_t getrootdirsectorscount(void)
{
	...
	if (!hasactive()) return 0;
	if (!isfat16type()) return 0;
	bytes = fat16->bytes_per_sector;
	entries = fat16->root_entries_count;
...
uint32_t getclustercount(void)
{
	...
	sector_per_cluster = fat16->sector_per_cluster;
...
uint32_t getsectornumber(uint32_t sector)
{
	...
	if (!hasactive()) return 0;
	if (!isfat16type()) return 0;
	reserved_sectors = fat16->reserved_sectors_count;
...
int main(int argc, char *argv[])
{
	...
	{
		printf("FAT16 List Directory\n");
		printf("   Created by Mario Freire\n");
		printf("\n");
		printf("Usage: fat16dir [image-file]\n");
		printf("Example: fat16dir harddisk.img\n");
		printf("\n");
	}
	return 0;
}
