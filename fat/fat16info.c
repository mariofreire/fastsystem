...
uint8_t loadbootsector(void)
{
		...
		if (!can_read) return 0;
        bootsector = (fat16_boot_sector_t*)BOOTSECTOR;	
		return 1;
}
...
uint8_t loadrootentries(void)
{
        ...
        while (next_dir_sector != 0)
        {
                ...
                while (next_dir_entry != 0)
                {
                        ...
                        fat16_file_entry_t *file_entry;
                        file_entry = (fat16_file_entry_t*)dir_entry_data;
...
uint8_t isfat16type(void)
{
        ...
        {
                case PARTITION_FAT16_UNDER_FAT32:
                case PARTITION_FAT16_OVER_FAT32:
                case PARTITION_FAT16_OVER_32MB_LBA:
                {
                     return 1;
                }
                break;
...
uint32_t getclusterstart(void)
{
		...
		lba_cluster = (main_partition->lba_start + bootsector->reserved_sector_count) + 
					  (bootsector->table_count * bootsector->table_size_16);
		return lba_cluster;
}
...
int main(int argc, char *argv[])
{
		...
		if (fileexists(imagefilename))
		{
			...
			if (imagefilesize >= SECTORSIZE)
			{
				...
				if (image_intialized)
				{
					...
					{
						printf("Cannot find FAT16 partition in image disk file '%s'.\n", imagefilename);
					}
					uninitimagedisk();
				}
				else
	...
	else
	{
		...
		printf("\n");
		printf("Usage: fat16info [image-file]\n");
		printf("Example: fat16info harddisk.img\n");
		printf("\n");
	}
	return 0;
}
