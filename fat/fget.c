									...
									if (path_sub.path[j].path[0] != 0)
									{
										...
										if (find_file != NULL)
										{
											...
											{							
												...
												if (data == NULL)
												{
													printf("Error: Cannot allocate file in memory.\n");
												}
												else {
													...
													if (fsec_cnt != fsecstotal) printf("File Read Error.\n");
														...
														int status = file_write(filename, data, find_file->size);
														if (status == 0)
														{
														...
														{
															if (status == 1) printf("Error: Cannot create file to write.\n");
															else if (status == 2) printf("Error: Cannot write file.\n");
															else printf("Error: Cannot get file to write.\n");
														}
													}
													free(data);													
												}
