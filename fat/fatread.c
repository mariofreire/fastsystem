		...
		{			
			    ...
			    (entries[filecount].attribute != F_ATTR_VOLMID))
			{
				if (entries[filecount].attribute != F_ATTR_LNGFNM)
				{
					...
					{
						if (entries[filecount].attribute & F_ATTR_RDONLY)
							printf("\033[%dm%s\033[m\n", 96, filename);
						else if (entries[filecount].attribute & F_ATTR_HIDDEN)
							printf("\033[%dm%s\033[m\n", 90, filename);
						else printf("\033[%dm%s\033[m\n", 37, filename);
					}
				}
				else
				{
								...
								for(j=0;j<path_sub.pathcount;j++)
								{
									if (path_sub.path[j].path[0] != 0)
									{
										...
										if (find_file != NULL)
										{
											...
											{
												printf("Directory '%s' located at 0x%08X\n", filename, (unsigned int)sectortobytes(sector));
												listdir(lsector);
											} else
											{												
												...
												fsec_cnt = sector;
												fsecstotal = fsec_cnt+bytestosector(filesizeondisk(find_file->size));	
												unsigned char *data = (unsigned char*)malloc(find_file->size+4096);
												...
												if (fsec_cnt != fsecstotal) printf("File Read Error.\n");
												else {
													dump_hex_address(sectortobytes(lsector), data, find_file->size);
												}
												free(data);
											}												
										}
